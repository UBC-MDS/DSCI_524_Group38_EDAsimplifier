...
def test_override_numeric_to_categorical():
    ...
    df = pd.DataFrame({"year": [2020, 2021, 2022], "genre": ["Pop", "Pop", "Rock"]})
    target_column = "genre"
    result = _ambiguous_columns_split(
        df, target_column, {"numeric": [], "categorical": ["year"]}
    )
...
def test_override_categorical_to_numeric():
    ...
    target_column = "genre"
    result = _ambiguous_columns_split(
        df, target_column, {"numeric": ["rank"], "categorical": []}
    )
...
def test_none_ambiguous_columns():
    ...
    assert set(result["numeric"].columns) == {"popularity", "track_id"}
    assert set(result["categorical"].columns) == {
        "genre",
        "is_explicit",
        "rank",
        "track_id",
...
def test_conflict_raises_error():
    ...
    with pytest.raises(ValueError):
        _ambiguous_columns_split(
            df, "genre", {"numeric": ["rank"], "categorical": ["rank"]}
        )
